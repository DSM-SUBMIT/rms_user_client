...
const saveReportSaga = function* (): any {
  const SUCCESS = `WRITE_REPORT/GET_SAVE_REPORT_SUCCESS`;
  const FAILURE = `WRITE_REPORT/GET_SAVE_REPORT_FAILURE`;
...
const submitReportSage = function* (): any {
  const SUCCESS = 'WRITE_REPORT/GET_SUBMIT_REPORT_SUCCESS';
  const FAILURE = 'WRITE_REPORT/GET_SUBMIT_REPORT_FAILURE';
  const state = yield select(getStateFunc);
  const request = WriteReportRequest(state);
  const accessToken = localStorage.getItem('access_token') || '';
  try {
